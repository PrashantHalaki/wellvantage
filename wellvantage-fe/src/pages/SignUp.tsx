import { useEffect } from 'react';
import { useNavigate } from 'react-router-dom';
import { useAuth } from '@/contexts/AuthContext';
import { Button } from '@/components/ui/button';
import { LeftCover } from '@/components/LeftCover';

export default function SignUp() {
  const { signInWithGoogle, user } = useAuth();
  const navigate = useNavigate();

  useEffect(() => {
    if (user) {
      if (user.onboarded) {
        navigate('/dashboard');
      } else {
        navigate('/onboarding');
      }
    }
  }, [user, navigate]);

  return (
    <div className='min-h-screen flex flex-col lg:flex-row'>
      {/* Left Panel - Brand */}
      <div className='lg:w-1/2 bg-primary flex items-center justify-center relative py-12 lg:py-0'>
        <LeftCover onBackClick={() => navigate('/')} />
      </div>

      {/* Right Panel - Sign Up Form */}
      <div className='flex-1 flex items-center justify-center p-4 lg:p-8 bg-background'>
        <div className='w-full max-w-md'>
          <div className='text-center mb-8 lg:mb-12'>
            <h2 className='text-3xl lg:text-4xl font-bold text-foreground mb-4'>Sign Up</h2>
            <p className='text-base lg:text-lg text-muted-foreground px-4'>
              Welcome! Manage, Track and Grow your Gym with Wellvantage.
            </p>
          </div>

          <Button
            onClick={signInWithGoogle}
            variant='outline'
            className='w-full h-12 lg:h-14 text-base lg:text-lg font-medium'
          >
            <svg className='w-6 h-6 mr-3' viewBox='0 0 24 24'>
              <path
                fill='#4285F4'
                d='M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z'
              />
              <path
                fill='#34A853'
                d='M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z'
              />
              <path
                fill='#FBBC05'
                d='M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z'
              />
              <path
                fill='#EA4335'
                d='M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z'
              />
            </svg>
            Continue with Google
          </Button>
        </div>
      </div>
    </div>
  );
}
